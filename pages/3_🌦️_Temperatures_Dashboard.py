...
unique_countries_list = list((temps_df['City']).unique())
...
if unique_countries_list is not None and len(selected_cities) > 0:
    ...
    plt.title('Temperature of the cities for the selected date')   # TODO
    ...
    for city in selected_cities:
        city_df = temps_df[temps_df["City"]== city]            # TODO
        city_df_period = city_df[(city_df['Date'] >= start_date) & (city_df['Date'] <= end_date)]    # TODO
        plt.hist(city_df_period["AvgTemperatureCelsius"],bins=50, alpha= 0.5, label=f'temp in {city}')                # TODO

    plt.title('TEMPERATURE DISTRIBUTION FOR SELECTED CITIES')   # TODO
    plt.xlabel('CELSIUS TEMPERATURE')  # TODO
